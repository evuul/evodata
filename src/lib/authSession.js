...
export function buildSessionUser(user, { isAdmin = Boolean(user?.isAdmin) } = {}) {
  ...
  return {
    ...
    subscriberPaymentSek: Number.isFinite(Number(user?.subscriberPaymentSek)) ? Number(user.subscriberPaymentSek) : null,
    subscriberStartedAt: user?.subscriberStartedAt ?? null,
