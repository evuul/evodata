...
test("includes the verified 14–18 September 2026 buyback report", () => {
  const current = readJson(new URL("../app/data/buybackData.json", import.meta.url));
  const latestWeek = current.filter((row) => row.Datum >= "2026-09-14" && row.Datum <= "2026-09-18");
  const programRows = current.filter((row) => row.Datum >= "2026-05-19" && row.Datum <= "2026-09-18");
  ...
  assert.equal(latestWeek.reduce((sum, row) => sum + row.Antal_aktier, 0), 778_000);
  assert.equal(
    latestWeek.reduce((sum, row) => sum + row.Transaktionsvärde, 0),
    695_440_095.6,
  );
  assert.equal(programRows.reduce((sum, row) => sum + row.Antal_aktier, 0), 15_991_100);
});
