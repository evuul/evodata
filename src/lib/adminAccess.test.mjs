...
test("the primary administrator remains recognized without environment configuration", () => {
  assert.equal(PRIMARY_ADMIN_EMAIL, "alexander.ek@live.se");
  assert.equal(isPrimaryAdminEmail(" Alexander.Ek@Live.se "), true);
  assert.equal(isConfiguredAdminEmail("alexander.ek@live.se", {}), true);
});
...
test("isConfiguredAdminEmail matches explicit ADMIN_EMAIL case-insensitively", () => {
  ...
  assert.equal(isConfiguredAdminEmail("ADMIN@EXAMPLE.COM", env), true);
  assert.equal(isConfiguredAdminEmail("alexander.ek@live.se", env), true);
