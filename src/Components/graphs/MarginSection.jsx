...
import { ResponsiveContainer, CartesianGrid, XAxis, YAxis, Tooltip, Label, LineChart, Line } from "recharts";
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center">
      ...
      {viewMode === "quarterly" && latestQuarter?.change && (
        ...
        >
          Förändring jämfört med {latestQuarter.year - 1} {latestQuarter.quarter}: {latestQuarter.change >= 0 ? "+" : ""}
          {latestQuarter.change} procentenheter
        </Typography>
      )}

      {viewMode === "yearly" && (marginYearlyChange?.length || 0) > 0 && marginYearlyChange[marginYearlyChange.length - 1].change && (
        <Typography
          variant="body2"
          color={marginYearlyChange[marginYearlyChange.length - 1].change >= 0 ? "#00e676" : "#ff1744"}
          sx={{ marginBottom: "10px", textAlign: "center", fontSize: { xs: "0.85rem", sm: "0.95rem" } }}
        >
          Förändring jämfört med {marginYearlyChange[marginYearlyChange.length - 1].year - 1}: {marginYearlyChange[marginYearlyChange.length - 1].change >= 0 ? "+" : ""}
        ...
        <LineChart data={viewMode === "quarterly" ? marginQuarterlyChange : marginYearlyChange} margin={{ top: 20, right: 20, bottom: isMobile ? 40 : 20, left: isMobile ? 10 : 40 }} connectNulls={false}>
          ...
          <Tooltip
            formatter={(value, name, props) => {
              ...
              return [value !== null ? `${value.toLocaleString("sv-SE")}%` : "Ingen data", change ? `Förändring: ${change >= 0 ? "+" : ""}${change} procentenheter` : "Ingen jämförelse tillgänglig"];
            }}
