...
export default function GamePlayersTrendChart() {
  ...
  async function fetchAllSeries(nDays) {
    ...
    await Promise.all(
      GAMES.map(async (g) => {
        try {
          const id = encodeURIComponent(g.id); // viktigt för crazy-time:a
    ...
    <Box sx={{ p:2 }}>
      <Box sx={{ display:"flex", justifyContent:"space-between", alignItems:"center", mb:1, gap:1, flexWrap:"wrap" }}>
        ...
        <Box sx={{ display:"flex", gap:0.5 }}>
          {[7,14,30,90].map(n=>(
            <Chip key={n} label={`${n}d`} size="small"
              onClick={()=>setDays(n)}
              sx={{ bgcolor: days===n ? "#3a3a3a" : "#2a2a2a", color:"#fff", border:"1px solid #3a3a3a", cursor:"pointer" }}
            />
          ))}
        </Box>
      </Box>
      ...
      <Box sx={{ width:"100%", height:300, background:"#1b1b1b", borderRadius:"10px", p:1 }}>
        {loading ? (
      ...
      <Typography variant="caption" sx={{ color:"#b0b0b0", display:"block", mt:1 }}>
        Dagligt snitt beräknas i Europe/Stockholm från sparade mätpunkter.
      </Typography>
    </Box>
  );
}