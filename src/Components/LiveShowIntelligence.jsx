...
const LiveShowIntelligence = ({ financialReports, averagePlayersData }) => {
  ...
  const baselineCandidates = useMemo(() => {
    const list = Object.entries(quarterlyPlayers)
      .map(([period, info]) => {
        const parts = periodKeyToParts(period);
        const revenue = revenueData[period];
        const isWhitelisted = BASELINE_WHITELIST.has(period);
        const playersForBaseline = info?.avgPlayers;
        if (
          !parts ||
  ...
  const forecastCalibrationFactor =
    baselineComputed?.source === "recent-average" ? 1 : calibrationFactor;
    ...
    return quarterPlayersList
      .map((period) => {
        ...
        const baselineForRow = period === forecastPeriod
          ? baselineRevenuePerPlayer
          : baselineForPeriod(period);
        const rowCalibrationFactor = Number.isFinite(actualRevenue) ? 1 : forecastCalibrationFactor;
        const est = Number.isFinite(baselineForRow)
          ? Math.round(playersForEstimate * baselineForRow * rowCalibrationFactor * 10) / 10
          : null;
        const diff =
  ...
  const trendText = useMemo(() => {
    if (!Number.isFinite(estimatedRevenue)) {
      return translate("Forecasten väntar på mer data.", "The forecast is waiting for more data.");
    }
    const refLabel = labelFromPeriod(lastYearSameQuarterPeriod);
    if (changeYoY.percent == null) {
      return translate(
        `Run-rate ligger runt ${formatMillion(estimatedRevenue)} €M mot ${refLabel}.`,
        `Run-rate is around ${formatMillion(estimatedRevenue)} €M versus ${refLabel}.`
      );
    }
    ...
    return translate(
      `Run-rate ligger ${descriptorSv} ${refLabel} med ${amountStr} (${percentStr}).`,
      `Run-rate is ${descriptorEn} ${refLabel} by ${amountStr} (${percentStr}).`
    );
  ...
  const combinedForecastEstimate = useMemo(() => {
    ...
    const liveValue =
      Number.isFinite(targetPlayers) && Number.isFinite(targetBaseline)
        ? Math.round(targetPlayers * targetBaseline * forecastCalibrationFactor * 10) / 10
        : null;
  ...
  return (
    ...
    >
      ...
      <Box sx={{ maxWidth: 560 }}>
          ...
          >
            {translate("Gameshow forecast", "Gameshow forecast")}
          </Typography>
          <Typography
            variant={isMobile ? "h5" : "h4"}
            sx={{ fontWeight: 700, color: "#f8fafc" }}
          >
            {translate("Spelshowarens omsättningsbrief", "Gameshow revenue brief")}
          </Typography>
          <Typography
            sx={{
                    ...
                    <RechartsTooltip
                      ...
                      formatter={(value, _name, { payload }) => {
                        ...
                        return [
                          ...
                          translate("Live-snitt", "Live average"),
                        ];
                      }}
