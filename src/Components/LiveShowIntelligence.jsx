...
const LiveShowIntelligence = ({ financialReports, averagePlayersData }) => {
      ...
      try {
        ...
        const averages = Array.isArray(json?.slugAverages)
          ? json.slugAverages
              ...
              .filter((row) =>
                typeof row.slug === "string" && Number.isFinite(row.avgPlayers)
              )
          : [];
  ...
  const quarterPlayersList = useMemo(() => {
    ...
    basePeriods.forEach((period) => {
      if (period && !unique.includes(period)) {
        unique.push(period);
      }
    });
    return unique;
  ...
  const qData = useMemo(() => {
    ...
    return mergedPlayersData
      .filter((item) => {
        ...
        return (
          date.getFullYear() === currentYear && getQuarter(date) === currentQuarter
        );
      })
  ...
  const adjustedAveragePlayers = Math.round(
    baseAveragePlayers * PLAYER_ADJUSTMENT_FACTOR
  );
  ...
  const revenueSoFar = Number.isFinite(estimatedRevenue)
    ? Math.round(
        (estimatedRevenue * quarterProgress.elapsedDays) /
          Math.max(quarterProgress.totalDays, 1) *
          10
      ) / 10
    : null;
  ...
  const tableRows = useMemo(() => {
    return quarterPlayersList
      .map((period) => {
        ...
        const actualRevenue = Number.isFinite(revenueData[period])
          ? revenueData[period]
          : null;
        ...
        const estimated = Number.isFinite(baselineRevenuePerPlayer)
          ? Math.round(playersForEstimate * baselineRevenuePerPlayer * 10) / 10
          : null;
        const diff =
          Number.isFinite(actualRevenue) && Number.isFinite(estimated)
            ? actualRevenue - estimated
            : null;
        const percent =
      ...
      .filter((row) => row.basePlayers > 0 || Number.isFinite(row.actual) || Number.isFinite(row.estimated));
  ...
  const trendText = (() => {
    ...
    const descriptor = change.percent >= 0 ? "över" : "under";
    return `Taktar ${descriptor} ${labelFromPeriod(previousPeriod)} med ${formatMillion(Math.abs(change.value))} €M (${Math.abs(change.percent).toFixed(1)}%).`;
  })();
  ...
  return (
    ...
    >
      ...
      <Grid container spacing={isMobile ? 2 : 3}>
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            <Typography sx={{ color: changeColor, fontSize: "0.95rem", mt: 0.5 }}>
              {change.percent == null
                ? "Inväntar jämförelsedata"
                : `${change.value >= 0 ? "+" : "-"}${formatMillion(Math.abs(change.value))} €M (${change.value >= 0 ? "+" : "-"}${Math.abs(change.percent).toFixed(1)}%) vs ${labelFromPeriod(previousPeriod)}`}
            </Typography>
      ...
      <Typography sx={{ color: "rgba(226,232,240,0.75)", mt: { xs: 3, md: 4 } }}>
        {trendText}
      </Typography>
