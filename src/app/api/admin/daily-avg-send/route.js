// Triggers administrator-approved daily player-average alerts.
...
export async function POST(request) {
  ...
  const force = payload?.force === false ? false : true;
  const targetYmd = String(payload?.targetYmd || "").trim();

  const origin = new URL(request.url).origin;
  const qs = new URLSearchParams();
  if (dryRun) qs.set("dryRun", "1");
  if (force) qs.set("force", "1");
  if (/^\d{4}-\d{2}-\d{2}$/.test(targetYmd)) qs.set("targetYmd", targetYmd);
  const url = `${origin}/api/alerts/daily-avg${qs.toString() ? `?${qs}` : ""}`;
  ...
  if (!res.ok) {
    return json({ error: data?.error || "Failed to trigger daily AVG send." }, { status: res.status });
  }
